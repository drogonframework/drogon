...
## Experimental C++26 reflection ORM

Drogon provides an experimental, opt-in code-first ORM interface based on C++26 reflection. Include the reflection header explicitly and annotate a public aggregate:
...
drogon::orm::ReflectiveMapper<User> mapper{client};
```

This interface requires GCC 16 and CMake 3.25 or newer. It is header-only and is not included by `<drogon/drogon.h>`, so existing C++17 and C++20 applications are unaffected. Set `BUILD_ORM_REFLECTION=ON` to enable the feature; the option is disabled by default.

The reflection interface is an initial alternative for basic CRUD, criteria, optional columns and automatically generated integral primary keys. It does not yet provide relations, DDL or migrations, dirty tracking, custom codecs, joins, pagination, ordering or partial updates. The established `drogon_ctl` workflow remains the schema-first choice for generating full ORM models from an existing database.
...
| BUILD_ORM_REFLECTION | Build experimental C++26 reflection orm | OFF |
