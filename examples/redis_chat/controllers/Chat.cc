...
void Chat::handleNewMessage(const WebSocketConnectionPtr& wsConnPtr,
  ...
  LOG_DEBUG << "WsClient new message from " << wsConnPtr->peerAddr().toIpPort();
  ...
  switch (operation) {
    ...
    case 1:  // Enter room
    {
      ...
      context->subscriber_->subscribe(room,
                                      [wsConnPtr](const std::string&,
...
void Chat::handleNewConnection(const HttpRequestPtr& req,
  ...
  redisLogin(
      [wsConnPtr, name, loginKey](int status) {
        ...
        if (status == 0) {
          wsConnPtr->send("ERROR: User [" + name + "] already logged in.");
          wsConnPtr->shutdown();
          return;
        }
