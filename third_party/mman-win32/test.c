...
int test_anon_map_readwrite() {
    void* map = mmap(
        NULL, 1024, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (map == MAP_FAILED) {
        printf(
            "mmap (MAP_ANONYMOUS, PROT_READ | PROT_WRITE) returned unexpected "
            "error: %d\n",
            errno);
        return -1;
    }
    ...
    *((unsigned char*)map) = 1;
    ...
    if (result != 0)
        printf(
            "munmap (MAP_ANONYMOUS, PROT_READ | PROT_WRITE) returned "
            "unexpected error: %d\n",
            errno);
...
int test_anon_map_readonly() {
    ...
    if (map == MAP_FAILED) {
        printf(
            "mmap (MAP_ANONYMOUS, PROT_READ) returned unexpected error: %d\n",
            errno);
        return -1;
    }
    ...
    *((unsigned char*)map) = 1;
    ...
    if (result != 0)
        printf(
            "munmap (MAP_ANONYMOUS, PROT_READ) returned unexpected error: %d\n",
            errno);
...
int test_anon_map_writeonly() {
    void* map =
        mmap(NULL, 1024, PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (map == MAP_FAILED) {
        printf(
            "mmap (MAP_ANONYMOUS, PROT_WRITE) returned unexpected error: %d\n",
            errno);
        return -1;
    }
    ...
    *((unsigned char*)map) = 1;
    ...
    if (result != 0)
        printf(
            "munmap (MAP_ANONYMOUS, PROT_WRITE) returned unexpected error: "
            "%d\n",
            errno);
...
int test_anon_map_readonly_nowrite() {
    ...
    if (*((unsigned char*)map) != 0)
        printf(
            "test_anon_map_readonly_nowrite (MAP_ANONYMOUS, PROT_READ) "
