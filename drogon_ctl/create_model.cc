...
bool drogon_ctl::ConvertMethod::shouldConvert(
...
void create_model::createModelClassFromPG(
  ...
  *client << "SELECT * "
      ...
      [&](const Result &r) {
        if (r.size() == 0) {
          std::cout << "    ---Can't create model from the table " << tableName
                    << ", please check privileges on the table." << std::endl;
          return;
        }
        for (Result::SizeType i = 0; i < r.size(); ++i) {
          ...
          if (type == "smallint") {
            ...
            if (!row["character_maximum_length"].isNull())
              info.colLength_ = row["character_maximum_length"].as<ssize_t>();
  ...
  *client << "SELECT \
  ...
  if (pkNumber == 1) {
    *client << "SELECT "
            ...
            << tableName << Mode::Blocking >>
        [&](bool isNull, const std::string &colName, const std::string &type) {
          if (isNull)
            return;
    ...
    for (auto &row : result) {
      ...
      if (type.find("int") != std::string::npos) {
        ...
        info.colLength_ = 8;
                 ...
                 type == "clob") {
        info.colType_ = "std::string";
      } else if (type.find("double") != std::string::npos || type == "real" ||
                 type == "float") {
        info.colType_ = "double";
        info.colLength_ = sizeof(double);
      } else if (type == "bool") {
...
void create_model::createModel(const std::string &path,
  ...
  if (dbType == "postgresql") {
    ...
    if (singleModelName.empty()) {
      auto tables = config["tables"];
      if (!tables || tables.size() == 0)
        createModelFromMysql(
            path, client, restfulApiConfig, relationships, convertMethods);
      else {
        for (int i = 0; i < (int)tables.size(); ++i) {
          auto tableName = tables[i].asString();
          std::transform(tableName.begin(),
    ...
    if (filename == "") {
      std::cerr << "Please configure filename in " << path << "/model.json "
                << std::endl;
      exit(1);
    }
...
void create_model::createRestfulAPIController(
  ...
  auto genBaseOnly = restfulApiConfig.get("generate_base_only", false).asBool();
  auto modelClassName = tableInfo.get<std::string>("className");
  std::regex regex("\\*");
  auto resource =
      std::regex_replace(restfulApiConfig.get("resource_uri", "/*").asString(),
                         regex,
                         modelClassName);
  ...
  }
  {
    ...
    try {
      auto templ = DrTemplateBase::newTemplate("restful_controller_base_h.csp");
      oHeadFile << templ->genText(data);
      templ = DrTemplateBase::newTemplate("restful_controller_base_cc.csp");
  ...
  if (!genBaseOnly) {
    ...
    try {
      ...
      oHeadFile << templ->genText(data);
      templ = DrTemplateBase::newTemplate("restful_controller_custom_cc.csp");
      oSourceFile << templ->genText(data);
    } catch (const std::exception &err) {
      std::cerr << err.what() << std::endl;
      exit(1);
    ...
    std::cout << "create a http restful API controller class: " << ctrlClassName
              << std::endl;
    std::cout << "file name: " << headFileName << ", " << sourceFilename
              << std::endl
              << std::endl;
