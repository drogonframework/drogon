...
static void parseLine(std::ofstream &oSrcFile,
  ...
  if (cxx_flag == 0) {
    ...
    if ((pos = line.find(cxx_lang)) != std::string::npos) {
      ...
      if (newLine.length() > 0)
        parseLine(
            oSrcFile, newLine, streamName, viewDataName, cxx_flag, returnFlag);
    } else {
    ...
    if ((pos = line.find(cxx_end)) != std::string::npos) {
      ...
      if (oldLine.length() > 0)
        parseLine(
            oSrcFile, oldLine, streamName, viewDataName, cxx_flag, returnFlag);
    } else {
  ...
  if (infile) {
    ...
    if (pos != std::string::npos) {
      ...
      if (!oHeadFile || !oSourceFile) {
        std::cerr << "Can't open " << headFileName << " or " << sourceFilename
                  << "\n";
        return -1;
      }
