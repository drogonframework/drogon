...
void press::createRequestAndClients() {
  ...
  for (size_t i = 0; i < numOfConnections_; ++i) {
    auto client = HttpClient::newHttpClient(host_, loopPool_->getNextLoop());
...
void press::sendRequest(const HttpClientPtr &client) {
  ...
  client->sendRequest(request,
                      [this, client, request](ReqResult r,
                        ...
                        if (processIndication_) {
                          ...
                          if (rec % 100000 == 0) {
                            std::cout << rec << " responses are received"
                                      << std::endl
                                      << std::endl;
                          }
                        }
...
void press::outputResults() {
  ...
  std::cout << "TOTALS:   " << numOfConnections_ << " connect, "
            << numOfRequests_ << " requests, " << statistics_.numOfGoodResponse_
            << " success, " << statistics_.numOfBadResponse_ << " fail"
            << std::endl;
