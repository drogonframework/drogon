...
void drogon_ctl::create_controller::handleCommand(
...
void drogon_ctl::create_controller::newSimpleControllerHeaderFile(
    std::ofstream &file,
...
void drogon_ctl::create_controller::newSimpleControllerSourceFile(
    std::ofstream &file,
    const std::string &className,
...
void drogon_ctl::create_controller::newWebsockControllerHeaderFile(
    std::ofstream &file,
    const std::string &className) {
  file << "#pragma once\n";
  file << "\n";
...
void drogon_ctl::create_controller::newWebsockControllerSourceFile(
    std::ofstream &file,
    const std::string &className,
    const std::string &filename) {
  file << "#include \"" << filename << ".h\"\n";
...
void drogon_ctl::create_controller::newHttpControllerHeaderFile(
    std::ofstream &file,
    const std::string &className) {
  file << "#pragma once\n";
  file << "\n";
...
void drogon_ctl::create_controller::newHttpControllerSourceFile(
    std::ofstream &file,
    const std::string &className,
    const std::string &filename) {
  file << "#include \"" << filename << ".h\"\n";
...
void drogon_ctl::create_controller::createController(
    std::vector<std::string> &httpClasses,
    ControllerType type) {
...
void drogon_ctl::create_controller::createARestfulController(
    const std::string &className,
  ...
  auto v = drogon::utils::splitString(className, "::");
  ...
  if (resource.empty()) {
    data.insert("ctlCommand",
                std::string("drogon_ctl create controller -r ") + className);
  } else {
    data.insert("ctlCommand",
                std::string("drogon_ctl create controller -r ") + className +
                    " --resource=" + resource);
  ...
  try {
    auto templ =
        drogon::DrTemplateBase::newTemplate("restful_controller_h.csp");
    oHeadFile << templ->genText(data);
    templ = drogon::DrTemplateBase::newTemplate("restful_controller_cc.csp");
