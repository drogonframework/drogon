...
class Relationship {
 public:
  ...
  Relationship(const Json::Value &relationship) {
    ...
    originalTableName_ = relationship.get("original_table_name", "").asString();
    ...
    targetTableAlias_ = relationship.get("target_table_alias", "").asString();
    enableReverse_ = relationship.get("enable_reverse", false).asBool();
    if (type_ == Type::ManyToMany) {
      auto &pivot = relationship["pivot_table"];
      if (pivot.isNull()) {
        throw std::runtime_error("ManyToMany relationship needs a pivot table");
      }
...
class create_model : public DrObject<create_model>, public CommandHandler {
 ...
 protected:
  void createModel(const std::string &path, const std::string &singleModelName);
