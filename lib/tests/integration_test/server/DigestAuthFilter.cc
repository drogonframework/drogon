  ...
  if (!auth_header.empty()) {
    ...
    if (httpHasAttribute(att_list, "username", &username) &&
      ...
      if (credentials.find(username) != credentials.end()) {
        ...
        std::string A1_middle_A2 = toLower(utils::getMd5(A1)) + ":" + nonce +
                                   ":" + toLower(utils::getMd5(A2));
        std::string calculated_response = toLower(utils::getMd5(A1_middle_A2));
        if (response == calculated_response) {
          // Passed
          ccb();
          return;
        } else {
          LOG_DEBUG << "invalid response " << response << ", calculated "
                    << calculated_response;
        }
      } else {
