...
void doTest(const HttpClientPtr &client, std::shared_ptr<test::Case> TEST_CTX) {
  ...
  client->sendRequest(
      ...
      [req, client, TEST_CTX](ReqResult result, const HttpResponsePtr &resp) {
        ...
        client->sendRequest(
            req1,
            [req1, TEST_CTX](ReqResult result, const HttpResponsePtr &resp1) {
              REQUIRE(result == ReqResult::Ok);
...
/// Test brotli
  ...
  client->sendRequest(
      ...
      [req, TEST_CTX, body](ReqResult result, const HttpResponsePtr &resp) {
        REQUIRE(result == ReqResult::Ok);
        CHECK(resp->getBody().length() == indexImplicitLen);
        CHECK(std::equal(body->begin(), body->end(), resp->getBody().begin()));
      });
  // Test file upload
  ...
  req->addHeader("Content-Encoding", "br");
  req->setMethod(drogon::Post);
  req->setBody(utils::brotliCompress(largeString.c_str(), largeString.size()));
  client->sendRequest(req,
                      [largeString, TEST_CTX](ReqResult result,
                                              const HttpResponsePtr &resp) {
                        REQUIRE(result == ReqResult::Ok);
