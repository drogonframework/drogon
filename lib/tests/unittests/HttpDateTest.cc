...
DROGON_TEST(HttpDate)
{
    ...
    CHECK(date.microSecondsSinceEpoch() / trantor::Date::MICRO_SECONDS_PER_SEC == 1591348778);
    ...
    CHECK(date.microSecondsSinceEpoch() / trantor::Date::MICRO_SECONDS_PER_SEC == epoch);
}
