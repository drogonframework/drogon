...
DROGON_TEST(CookieSameSite) {
  ...
  while (seq.i != seq.valid_sameSite_values.end()) {
    ...
    if (seq.sessionCookie.getValue() != "") {
      // add session cookie
      req->addCookie(seq.sessionCookie.getKey(), seq.sessionCookie.getValue());
    }  // endif

    client->sendRequest(
        req, [TEST_CTX, &seq, &p1](ReqResult res, const HttpResponsePtr &resp) {
          ...
          LOG_INFO << "Client: cookie-value == " << cookie.value()
                   ...
                   << ", new  value == " << (*json)["new value"].asString()
                   << ", received value == "
                   << Cookie::convertSameSite2String(cookie.getSameSite());
          seq.sessionCookie = resp->getCookie("JSESSIONID");
          CHECK(*seq.i == Cookie::convertSameSite2String(cookie.getSameSite()));
