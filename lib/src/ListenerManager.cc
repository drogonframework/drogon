...
void ListenerManager::createListeners(
  ...
  for (size_t i = 0; i < ioLoops.size(); ++i) {
    for (auto const &listener : listeners_) {
      ...
      std::shared_ptr<HttpServer> serverPtr = std::make_shared<HttpServer>(
          ioLoops[i], listenAddress, "drogon", syncAdvices, preSendingAdvices);
  ...
  if (!listeners_.empty()) {
    listeningThread_ = std::make_unique<EventLoopThread>("DrogonListeningLoop");
    listeningThread_->run();
    for (auto const &listener : listeners_) {
      auto ip = listener.ip_;
      bool isIpv6 = (ip.find(':') != std::string::npos);
      auto serverPtr =
          std::make_shared<HttpServer>(listeningThread_->getLoop(),
