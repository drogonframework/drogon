...
static std::optional<SettingsFrame> parseSettingsFrame(const uint8_t* ptr, size_t length)
{
    if(length % 6 != 0)
    {
    ...
    for(size_t i = 0; i < length; i += 6)
    {
        ...
        uint32_t value = ptr[i + 2] << 24 | ptr[i + 3] << 16 | ptr[i + 4] << 8 | ptr[i + 5];
...
static std::optional<WindowUpdateFrame> parseWindowUpdateFrame(const uint8_t* ptr, size_t length)
{
    if(length != 4)
    {
    ...
    frame.windowSizeIncrement = (ptr[0] << 24 | ptr[1] << 16 | ptr[2] << 8 | ptr[3]) & 0x7fffffff;
    LOG_TRACE << "Window update frame: windowSizeIncrement=" << frame.windowSizeIncrement;
    return frame;
}
...
static size_t parseH2Frame(trantor::MsgBuffer *msg)
{
    ...
    LOG_TRACE << "H2 frame: length=" << length << " type=" << (int)type << " flags=" << (int)flags << " streamId=" << streamId;
