...
void HttpResponseImpl::generateBodyFromJson() const {
  ...
  bodyPtr_ =
      std::make_shared<HttpMessageStringBody>(writeString(builder, *jsonPtr_));
}
  ...
  if (resp) {
    if (loop && loop->index() < app().getThreadNum()) {
      ...
      return HttpResponsePtr{
          new HttpResponseImpl(*static_cast<HttpResponseImpl *>(resp.get()))};
    }
  } else {
    if (loop && loop->index() < app().getThreadNum()) {
      ...
      std::call_once(threadOnce, [] {
        thread404Pages.init([](drogon::HttpResponsePtr &resp,
          ...
          if (HttpAppFrameworkImpl::instance().isUsingCustomErrorHandler()) {
            ...
            resp = HttpResponse::newHttpViewResponse("drogon::NotFound", data);
...
void HttpResponseImpl::makeHeaderString(trantor::MsgBuffer &buffer) {
  ...
  if (!passThrough_) {
    ...
    if (HttpAppFrameworkImpl::instance().sendServerHeader()) {
      buffer.append(HttpAppFrameworkImpl::instance().getServerHeaderString());
    }
  }
    ...
    if (!passThrough_ &&
      ...
      if (datePos_ != static_cast<size_t>(-1)) {
        ...
        bool isDateChanged = ((now.microSecondsSinceEpoch() /
                               MICRO_SECONDS_PRE_SEC) != httpStringDate_);
        assert(httpString_);
        if (isDateChanged) {
          httpStringDate_ =
          ...
          httpString_ = std::make_shared<trantor::MsgBuffer>(*httpString_);
    ...
    for (size_t i = 0; i < values.size(); ++i) {
      ...
      if (i == 0) {
        ...
        if (cookie_name == "path") {
          ...
          cookie.setSameSite(cookie.convertString2SameSite(cookie_value));
...
void HttpResponseImpl::parseJson() const {
  ...
  if (bodyPtr_) {
    ...
    if (!reader->parse(bodyPtr_->data(),
      ...
      jsonPtr_.reset();
      jsonParsingErrorPtr_ = std::make_shared<std::string>(std::move(errs));
    } else {
      jsonParsingErrorPtr_.reset();
    }
  } else {
    jsonPtr_.reset();
    jsonParsingErrorPtr_ = std::make_shared<std::string>("empty response body");
  }
}
