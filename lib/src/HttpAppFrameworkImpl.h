...
class HttpAppFrameworkImpl final : public HttpAppFramework {
 public:
  ...
  std::shared_ptr<PluginBase> getSharedPlugin(const std::string &name) override;
  ...
  HttpAppFramework &registerWebSocketController(
      ...
      const std::vector<internal::HttpConstraint> &filtersAndMethods) override;
  ...
  HttpAppFramework &setCustomErrorHandler(
      std::function<HttpResponsePtr(HttpStatusCode)> &&resp_generator) override;
  ...
  HttpAppFramework &loadConfigJson(const Json::Value &data) noexcept(
      false) override;
  HttpAppFramework &loadConfigJson(Json::Value &&data) noexcept(false) override;
  ...
  const std::function<HttpResponsePtr(HttpStatusCode)> &getCustomErrorHandler()
      const override;
 ...
 private:
  ...
  void registerHttpControllerViaRegex(
      ...
      const std::vector<std::string> &filters,
      const std::string &handlerName) override;
  void onAsyncRequest(const HttpRequestImplPtr &req,
                      std::function<void(const HttpResponsePtr &)> &&callback);
  ...
  std::pair<unsigned int, std::string> floatPrecisionInJson_{0, "significant"};
  ...
  std::vector<
      std::function<void(const HttpRequestPtr &, const HttpResponsePtr &)>>
      preSendingAdvices_;
  std::vector<std::function<void(const HttpRequestPtr &)>> preRoutingObservers_;
