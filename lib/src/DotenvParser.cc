...
void DotenvParser::parse(std::ifstream &infile)
{
    ...
    std::vector<std::string> lines =
        drogon::utils::splitString(content.str(), "\n");
    std::string ns;
    ...
    for (const auto &line : lines)
    {
        ...
        if (isEqual(ns, "app"))
        {
            ...
            {
                configJsonValue_["app"]["number_of_threads"] =
                    std::stoi(pair.second);
            }
            else if (isEqual(pair.first, "enable_session"))
            {
                configJsonValue_["app"]["enable_session"] =
                    isEqual(pair.second, "true");
            }
            else if (isEqual(pair.first, "session_timeout"))
            {
                configJsonValue_["app"]["session_timeout"] =
                    std::stoi(pair.second);
            }
            ...
            {
                configJsonValue_["app"]["enable_server_header"] =
                    isEqual(pair.second, "true");
            }
            else if (isEqual(pair.first, "enable_date_header"))
            {
                configJsonValue_["app"]["enable_date_header"] =
                    isEqual(pair.second, "true");
            }
            else if (isEqual(pair.first, "keepalive_requests"))
            {
                configJsonValue_["app"]["keepalive_requests"] =
                    std::stoi(pair.second);
            }
            else if (isEqual(pair.first, "pipelining_requests"))
            {
                configJsonValue_["app"]["pipelining_requests"] =
                    std::stoi(pair.second);
            }
            else if (isEqual(pair.first, "gzip_static"))
            {
                configJsonValue_["app"]["gzip_static"] =
                    isEqual(pair.second, "true");
            }
            else if (isEqual(pair.first, "br_static"))
            {
                configJsonValue_["app"]["br_static"] =
                    isEqual(pair.second, "true");
            }
            ...
            {
                configJsonValue_["app"]["client_max_memory_body_size"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "client_max_websocket_message_size"))
            {
                configJsonValue_["app"]["client_max_websocket_message_size"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "reuse_port"))
            {
                configJsonValue_["app"]["reuse_port"] =
                    isEqual(pair.second, "true");
            }
            ...
            {
                configJsonValue_["app"]["use_implicit_page"] =
                    isEqual(pair.second, "true");
            }
        ...
        else if (isEqual(ns, "log"))
        {
            ...
            {
                configJsonValue_["app"]["log"]["logfile_base_name"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "log_size_limit"))
            {
                configJsonValue_["app"]["log"]["log_size_limit"] =
                    std::stoi(pair.second);
            }
        ...
        else if (isEqual(ns, "listeners"))
        {
            ...
            {
                configJsonValue_["listeners"][listenersIndex]["address"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "port"))
            {
                configJsonValue_["listeners"][listenersIndex]["port"] =
                    std::stoi(pair.second);
            }
            else if (isEqual(pair.first, "https"))
            {
                configJsonValue_["listeners"][listenersIndex]["https"] =
                    isEqual(pair.second, "true");
            }
            else if (isEqual(pair.first, "cert"))
            {
                configJsonValue_["listeners"][listenersIndex]["cert"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "key"))
            {
                configJsonValue_["listeners"][listenersIndex]["key"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "use_old_tls"))
            {
                configJsonValue_["listeners"][listenersIndex]["use_old_tls"] =
                    isEqual(pair.second, "true");
            }
        }
        else if (isEqual(ns, "db_clients"))
        {
            ...
            {
                configJsonValue_["db_clients"][dbClientsIndex]["rdbms"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "host"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["host"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "port"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["port"] =
                    std::stoi(pair.second);
            }
            else if (isEqual(pair.first, "dbname"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["dbname"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "user"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["user"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "passwd"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["passwd"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "password"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["password"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "connection_number"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]
                                ["connection_number"] = std::stoi(pair.second);
            }
            ...
            {
                configJsonValue_["db_clients"][dbClientsIndex]["name"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "filename"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["filename"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "is_fast"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["is_fast"] =
                    isEqual(pair.second, "true");
            }
            else if (isEqual(pair.first, "characterSet"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["characterSet"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "client_encoding"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]
                                ["client_encoding"] = pair.second;
            }
            else if (isEqual(pair.first, "timeout"))
            {
                configJsonValue_["db_clients"][dbClientsIndex]["timeout"] =
                    std::stod(pair.second);
            }
        }
        else if (isEqual(ns, "redis_clients"))
        {
            ...
            {
                configJsonValue_["redis_clients"][redisClientsIndex]["host"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "port"))
            {
                configJsonValue_["redis_clients"][redisClientsIndex]["port"] =
                    std::stoi(pair.second);
            }
            else if (isEqual(pair.first, "passwd"))
            {
                configJsonValue_["redis_clients"][redisClientsIndex]["passwd"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "password"))
            {
                configJsonValue_["redis_clients"][redisClientsIndex]
            ...
            {
                configJsonValue_["redis_clients"][redisClientsIndex]["name"] =
                    pair.second;
            }
            else if (isEqual(pair.first, "is_fast"))
            {
                configJsonValue_["redis_clients"][redisClientsIndex]
                                ["is_fast"] = isEqual(pair.second, "true");
            }
            else if (isEqual(pair.first, "timeout"))
            {
                configJsonValue_["redis_clients"][redisClientsIndex]
                                ["timeout"] = std::stod(pair.second);
            }
            else if (isEqual(pair.first, "db"))
            {
                configJsonValue_["redis_clients"][redisClientsIndex]["db"] =
                    std::stoi(pair.second);
            }
        }
    }
