...
void HttpRequestImpl::parseJson() const {
  ...
  if (contentType_ == CT_APPLICATION_JSON ||
    ...
    if (!reader->parse(
            input.data(), input.data() + input.size(), jsonPtr_.get(), &errs)) {
      LOG_DEBUG << errs;
      jsonPtr_.reset();
      jsonParsingErrorPtr_ = std::make_unique<std::string>(std::move(errs));
    } else {
      jsonParsingErrorPtr_.reset();
    }
  } else {
    jsonPtr_.reset();
    jsonParsingErrorPtr_ = std::make_unique<std::string>("content type error");
  }
}
...
void HttpRequestImpl::appendToBuffer(trantor::MsgBuffer *output) const {
  ...
  if (!passThrough_ && !parameters_.empty() &&
    ...
    if (contentType_ != CT_APPLICATION_X_FORM) {
      auto ret =
          std::find(output->peek(), (const char *)output->beginWrite(), '?');
