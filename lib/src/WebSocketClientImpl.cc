...
void WebSocketClientImpl::connectToServerInLoop() {
        ...
        [thisPtr = shared_from_this(),
         hasIpv6Address](const trantor::InetAddress &addr) {
          thisPtr->loop_->runInLoop([thisPtr, addr, hasIpv6Address]() {
            ...
            if ((thisPtr->serverAddr_.ipNetEndian() != 0 || hasIpv6Address) &&
  ...
  if ((serverAddr_.ipNetEndian() != 0 || hasIpv6Address) &&
    ...
    requestCallback_(ReqResult::BadServerAddress, nullptr, shared_from_this());
    return;
  }
}
...
#endif
    upgraded_ = true;
    websockConnPtr_ = std::make_shared<WebSocketConnectionImpl>(connPtr, false);
