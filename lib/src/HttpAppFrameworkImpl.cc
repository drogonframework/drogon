...
HttpAppFramework &HttpAppFrameworkImpl::setSSLFiles(
...
void HttpAppFrameworkImpl::run() {
  ...
  if (!libFilePaths_.empty()) {
    sharedLibManagerPtr_ =
        std::make_unique<SharedLibManager>(libFilePaths_, libFileOutputPath_);
  }
#endif
...
void HttpAppFrameworkImpl::findSessionForRequest(
...
orm::DbClientPtr HttpAppFrameworkImpl::getFastDbClient(
    const std::string &name) {
