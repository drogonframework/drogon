...
const std::map<std::string, std::string> &MultiPartParser::getParameters()
    const {
  return parameters_;
}
...
int MultiPartParser::parseEntity(const char *begin, const char *end) {
  ...
  while (pos != headEnd) {
    ...
    if (key == "content-disposition") {
      ...
      auto fileNamePos = std::search(nameEnd, valueEnd, fileName, fileName + 9);
      if (fileNamePos == valueEnd) {
        ...
        filePtr->setFile(headEnd + 2, static_cast<size_t>(end - headEnd - 2));
      }
