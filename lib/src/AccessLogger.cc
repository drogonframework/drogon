...
void AccessLogger::createLogFunctions(std::string format) {
  ...
  if (!rawString.empty()) {
    logFunctions_.emplace_back(
        [rawString = std::move(rawString)](trantor::LogStream &stream,
                                           const drogon::HttpRequestPtr &,
                                           const drogon::HttpResponsePtr &) {
          stream << rawString << "\n";
        });
...
AccessLogger::LogFunction AccessLogger::newLogFunction(
  ...
  if (placeholder.find("$http_") == 0 && placeholder.size() > 6) {
    ...
    return
        [headerName = std::move(headerName)](trantor::LogStream &stream,
                                             const drogon::HttpRequestPtr &req,
                                             const drogon::HttpResponsePtr &) {
  ...
  if (placeholder.find("$cookie_") == 0 && placeholder.size() > 8) {
    ...
    return
        [cookieName = std::move(cookieName)](trantor::LogStream &stream,
                                             const drogon::HttpRequestPtr &req,
                                             const drogon::HttpResponsePtr &) {
  ...
  return
      [placeholder](trantor::LogStream &stream,
                    const drogon::HttpRequestPtr &,
                    const drogon::HttpResponsePtr &) { stream << placeholder; };
}
...
void AccessLogger::outputProcessingTime(trantor::LogStream &stream,
  ...
  auto duration = end.microSecondsSinceEpoch() - start.microSecondsSinceEpoch();
