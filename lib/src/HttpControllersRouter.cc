...
void HttpControllersRouter::init(
  ...
  for (auto &router : ctrlVector_) {
    router.regex_ =
        std::regex(router.pathParameterPattern_, std::regex_constants::icase);
...
HttpControllersRouter::getHandlersInfo() const {
  ...
  auto gatherInfo = [&](const auto &item) {
    for (size_t i = 0; i < Invalid; ++i) {
      if (item.binders_[i]) {
        auto description = item.binders_[i]->handlerName_.empty()
                               ? std::string("Handler: ") +
  ...
  while (std::regex_search(tmpPath, results, regex)) {
    if (results.size() > 1) {
      ...
      if (!result.empty() &&
        ...
        if (!std::all_of(places.begin(), places.end(), [place](size_t i) {
              return i != place;
            })) {
          LOG_ERROR << "Parameter placeholders are duplicated: index=" << place;
          LOG_ERROR << "Path pattern: " << path;
          exit(1);
        }
        ...
        if (!result.empty() &&
          ...
          if (!std::all_of(places.begin(),
              ...
              !all_of(parametersPlaces.begin(),
                      ...
                      [place](const std::pair<std::string, size_t> &item) {
          ...
          if (std::regex_match(result, regexResult, regNumberAndName)) {
            ...
            if (!std::all_of(places.begin(),
                ...
                !all_of(parametersPlaces.begin(),
                        parametersPlaces.end(),
                        [placeIndex](
                            const std::pair<std::string, size_t> &item) {
            ...
            parametersPlaces.emplace_back(results[1].str(), placeIndex);
          }
        }
        ++placeIndex;
      }
  ...
  if (postRoutingAdvices_.empty()) {
    if (!binder->filters_.empty()) {
      ...
      doPreHandlingAdvices(
          binder, routerItem, req, std::move(result), std::move(callback));
    }
  } else {
    auto callbackPtr =
        std::make_shared<std::function<void(const HttpResponsePtr &)>>(
    ...
    doAdvicesChain(
        ...
        [this,
         ...
         result = std::move(matchResult)]() {
          doControllerHandler(
              ctrlBinderPtr, routerItem, req, result, std::move(*callbackPtr));
        });
  }
}
