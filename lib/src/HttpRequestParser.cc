...
void HttpRequestParser::shutdownConnection(HttpStatusCode code) {
  ...
  if (connPtr) {
    connPtr->send(
        utils::formattedString("HTTP/1.1 %d %s\r\nConnection: close\r\n\r\n",
                               code,
  ...
  return std::shared_ptr<HttpRequestImpl>(ptr, [weakPtr](HttpRequestImpl *p) {
    ...
    if (thisPtr) {
      if (thisPtr->loop_->isInLoopThread()) {
        p->reset();
        thisPtr->requestsPool_.emplace_back(thisPtr->makeRequestForPool(p));
      } else {
  ...
  while (true) {
    switch (status_) {
      ...
      case HttpRequestParseStatus::kExpectHeaders: {
        ...
        if (expect == "100-continue" &&
          ...
          if (currentContentLength_ >
            ...
            auto httpString =
                static_cast<HttpResponseImpl *>(resp.get())->renderToBuffer();
            reset();
      ...
      case HttpRequestParseStatus::kExpectBody: {
        size_t bytesToConsume = currentContentLength_ <= buf->readableBytes()
