  ...
  if (iter != simpleCtrlMap_.end()) {
    ...
    if (postRoutingAdvices_.empty()) {
      if (!filters.empty()) {
        ...
        doPreHandlingAdvices(binder, ctrlInfo, req, std::move(callback));
      }
      return;
    } else {
      auto callbackPtr =
      ...
      doAdvicesChain(
          ...
          req,
          callbackPtr,
          [callbackPtr, &filters, req, &ctrlInfo, this, &binder]() mutable {
            if (!filters.empty()) {
              filters_function::doFilters(
                  filters,
                  req,
  ...
  if (controller) {
    ...
    try {
      controller->asyncHandleHttpRequest(
          req,
          [this, req, callback, &ctrlBinderPtr](const HttpResponsePtr &resp) {
            auto newResp = resp;
  ...
  if (preHandlingAdvices_.empty()) {
    ...
    doAdvicesChain(
        ...
        [this, ctrlBinderPtr, req, callbackPtr]() {
          doControllerHandler(ctrlBinderPtr, req, std::move(*callbackPtr));
        });
  }
}
