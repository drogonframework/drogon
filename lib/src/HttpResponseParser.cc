...
bool HttpResponseParser::processResponseLine(const char *begin,
                                             const char *end) {
    ...
    if (status_ == HttpResponseParseStatus::kExpectResponseLine) {
      ...
      if (crlf) {
        ...
        if (colon != crlf) {
          ...
          const std::string &len = responsePtr_->getHeaderBy("content-length");
          ...
          if (!len.empty()) {
            ...
            if (encode == "chunked") {
              ...
              if (responsePtr_->statusCode() == k204NoContent ||
                  (responsePtr_->statusCode() == k101SwitchingProtocols &&
                       [this]() -> bool {
      ...
      if (buf->readableBytes() >= (currentChunkLength_ + 2)) {
        if (*(buf->peek() + currentChunkLength_) == '\r' &&
          ...
          responsePtr_->bodyPtr_->append(buf->peek(), currentChunkLength_);
