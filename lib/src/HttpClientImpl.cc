      ...
      [weakPtr](const trantor::TcpConnectionPtr &connPtr) {
        ...
        if (connPtr->connected()) {
          ...
          while (thisPtr->pipeliningCallbacks_.size() <=
            ...
            thisPtr->sendReq(connPtr, thisPtr->requestsBuffer_.front().first);
...
void HttpClientImpl::sendRequestInLoop(const drogon::HttpRequestPtr &req,
  ...
  if (!tcpClientPtr_) {
    ...
    dns_ = true;
    if (!resolverPtr_) {
      resolverPtr_ = trantor::Resolver::newResolver(loop_, kDefaultDNSTimeout);
    }
    auto thisPtr = shared_from_this();
    resolverPtr_->resolve(domain_, [thisPtr](const trantor::InetAddress &addr) {
