 ...
 public:
  ...
  T *getPlugin() {
    static_assert(IsPlugin<T>::value,
                  ...
                  "PluginBase");
    assert(isRunning());
    static auto pluginPtr = dynamic_cast<T *>(getPlugin(T::classTypeName()));
    return pluginPtr;
  }
  ...
  virtual HttpAppFramework &registerSyncAdvice(
      const std::function<HttpResponsePtr(const HttpRequestPtr &)> &advice) = 0;
 ...
 */
   ...
   * This operation can be performed by an option in the configuration file.
   */
  virtual HttpAppFramework &setLogPath(const std::string &logPath,
   ...
   */
  virtual const std::pair<unsigned int, std::string> &getFloatPrecisionInJson()
      const noexcept = 0;
   ...
   * This operation can be performed by an option in the configuration file.
   */
  virtual HttpAppFramework &createDbClient(const std::string &dbType,
