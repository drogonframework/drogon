...
Drogon 提供了一個以 C++26 反射為基礎、可選擇啟用的實驗性程式碼優先 ORM 介面。明確包含反射標頭檔，並為一個公開聚合型別加上註解：
...
此介面需要 GCC 16 和 CMake 3.25 或更新版本。它僅由標頭檔實作，且不會被 `<drogon/drogon.h>` 包含，因此不會影響現有的 C++17 和 C++20 應用程式。設定 `BUILD_ORM_REFLECTION=ON` 即可啟用此功能；該選項預設為關閉。

反射介面目前是一個初步實作，可替代現有方案完成基本 CRUD、查詢條件、可選欄位和由資料庫自動產生的整數主鍵。它尚未提供關聯、DDL 或遷移、異動追蹤、自訂編解碼器、聯結查詢、分頁、排序或部分更新。若要從現有資料庫產生完整的 ORM 模型，既有的 `drogon_ctl` 工作流程仍是綱要優先的選擇。
