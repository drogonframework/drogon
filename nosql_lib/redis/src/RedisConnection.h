...
namespace drogon {
namespace nosql {
enum class ConnectStatus { kNone = 0, kConnecting, kConnected, kEnd };
...
class RedisConnection : public trantor::NonCopyable,
 ...
 public:
  ...
  static std::string getFormattedCommand(const std::string_view &command,
    ...
    if (len == -1) {
      throw RedisException(RedisErrorCode::kInternalError, "Out of memory");
    } else if (len == -2) {
 ...
 private:
  ...
  std::function<void(std::shared_ptr<RedisConnection> &&)> disconnectCallback_;
