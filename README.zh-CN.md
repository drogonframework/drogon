...
Drogon 提供了一个基于 C++26 反射、可选择启用的实验性代码优先 ORM 接口。显式包含反射头文件，并为一个公有聚合类型添加注解：
...
此接口需要 GCC 16 和 CMake 3.25 或更高版本。它仅由头文件实现，且不会被 `<drogon/drogon.h>` 包含，因此不会影响现有的 C++17 和 C++20 应用程序。设置 `BUILD_ORM_REFLECTION=ON` 可启用此功能；该选项默认关闭。

反射接口目前是一个初步实现，可替代现有方案完成基本 CRUD、查询条件、可选列和数据库自动生成的整数主键。它暂不支持关系、DDL 或迁移、脏数据跟踪、自定义编解码器、连接查询、分页、排序或部分更新。对于从现有数据库生成完整 ORM 模型，现有的 `drogon_ctl` 工作流仍是模式优先方案。
