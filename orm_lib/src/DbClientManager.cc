  ...
  for (auto &dbInfo : dbInfos_) {
    if (dbInfo.isFast_) {
      ...
      if (dbInfo.dbType_ == drogon::orm::ClientType::PostgreSQL ||
        ...
        dbFastClientsMap_[dbInfo.name_] = IOThreadStorage<orm::DbClientPtr>();
        dbFastClientsMap_[dbInfo.name_].init(
            [&](orm::DbClientPtr &c, size_t idx) {
              assert(idx == ioloops[idx]->index());
              LOG_TRACE << "create fast database client for the thread " << idx;
              c = std::shared_ptr<orm::DbClient>(
                  new drogon::orm::DbClientLockFree(dbInfo.connectionInfo_,
                                                    ioloops[idx],
                                                    dbInfo.dbType_,
...
#if USE_MYSQL
        dbClientsMap_[dbInfo.name_] =
            drogon::orm::DbClient::newMysqlClient(dbInfo.connectionInfo_,
                                                  dbInfo.connectionNumber_);
        if (dbInfo.timeout_ > 0.0) {
          dbClientsMap_[dbInfo.name_]->setTimeout(dbInfo.timeout_);
        }
...
DbClientManager::~DbClientManager() {
  ...
  for (auto &pair : dbFastClientsMap_) {
    pair.second.init([](DbClientPtr &clientPtr, size_t index) {
      ...
      std::promise<void> p;
      auto f = p.get_future();
      drogon::getIOThreadStorageLoop(index)->runInLoop([&clientPtr, &p]() {
        clientPtr->closeAll();
        p.set_value();
      });
      f.get();
