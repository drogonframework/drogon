    ...
    if (type_ != ClientType::PostgreSQL) {
      ...
      for (size_t i = 0; i < connections_.size(); ++i) {
        ...
        if (connectionPos_ >= connections_.size())
          connectionPos_ = 0;
        if (transSet_.empty() || transSet_.find(conn) == transSet_.end()) {
              ...
              [resultCallback = std::move(resultCallback),
               this](const Result &r) {
                if (sqlCmdBuffer_.empty()) {
                  ...
                  loop_->queueInLoop(
                      [resultCallback = std::move(resultCallback), r]() {
                        resultCallback(r);
                      });
                }
              },
  ...
  auto cmdPtr = std::make_shared<SqlCmd>(
      ...
      [resultCallback = std::move(resultCallback), this](const Result &r) {
        if (sqlCmdBuffer_.empty()) {
          ...
          loop_->queueInLoop([resultCallback = std::move(resultCallback), r]() {
            resultCallback(r);
          });
        }
      },
