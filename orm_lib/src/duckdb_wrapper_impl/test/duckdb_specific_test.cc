...
void testVectorizedOperations(DbClientPtr client)
{
    ...
    // 创建测试表
    client->execSqlAsync(
        "CREATE TABLE IF NOT EXISTS vector_test (id INTEGER, value DOUBLE, "
        "category VARCHAR)",
        [](const Result &result) { printTestResult("创建向量化测试表", true); },
    ...
    client->execSqlAsync(
        ...
        "SUM(value) as sum_value FROM vector_test GROUP BY category ORDER BY "
        "category",
        [](const Result &result) {
            ...
            if (success)
            {
                for (const auto &row : result)
                {
                    ...
                    // 验证数据
                    if (category == "A" && count > 0 && avg_value > 0 &&
                        sum_value > 0)
                    {
                        success = true;
                    }
                    else if (category == "B" && count > 0 && avg_value > 0 &&
                             sum_value > 0)
                    {
                        success = true;
                    }
                    else if (category == "C" && count > 0 && avg_value > 0 &&
                             sum_value > 0)
                    {
                        success = true;
                    }
    ...
    // 测试窗口函数
    client->execSqlAsync(
        "SELECT id, value, category, ROW_NUMBER() OVER (PARTITION BY category "
        "ORDER BY value) as row_num FROM vector_test LIMIT 20",
        [](const Result &result) {
...
void testAnalyticalFunctions(DbClientPtr client)
{
    ...
    // 创建时间序列数据
    client->execSqlAsync(
        "CREATE TABLE IF NOT EXISTS time_series (date DATE, value DOUBLE, "
        "category VARCHAR)",
        [](const Result &result) { printTestResult("创建时间序列表", true); },
        [](const std::exception_ptr &e) {
    ...
    // 测试移动平均
    client->execSqlAsync(
        "SELECT date, value, AVG(value) OVER (ORDER BY date ROWS BETWEEN 2 "
        "PRECEDING AND CURRENT ROW) as moving_avg FROM time_series ORDER BY "
        "date LIMIT 10",
        [](const Result &result) {
    ...
    // 测试排名函数
    client->execSqlAsync(
        "SELECT date, value, RANK() OVER (ORDER BY value DESC) as rank_value "
        "FROM time_series ORDER BY date LIMIT 10",
        [](const Result &result) {
            bool success = result.size() == 10;
            if (success)
            {
...
void testComplexJsonOperations(DbClientPtr client)
{
    ...
    // 创建复杂JSON表
    client->execSqlAsync(
        "CREATE TABLE IF NOT EXISTS complex_json_test (id INTEGER, data JSON)",
    ...
    {
        client->execSqlAsync(
            "INSERT INTO complex_json_test VALUES (?, ?)",
            [](const Result &result) {
                // 成功回调
            },
            [](const std::exception_ptr &e) {
    ...
    // 测试JSON路径查询
    client->execSqlAsync(
        "SELECT id, data->>'$.user.name' as name, data->>'$.user.profile.age' "
        "as age FROM complex_json_test",
        [](const Result &result) {
    ...
    // 测试JSON数组操作
    client->execSqlAsync(
        "SELECT id, data->>'$.user.orders[0].amount' as first_order_amount "
        "FROM complex_json_test",
        [](const Result &result) {
            bool success = result.size() == 2;
            if (success)
            {
    ...
    // 测试JSON聚合
    client->execSqlAsync(
        "SELECT COUNT(*) as total_users, AVG(CAST(data->>'$.user.profile.age' "
        "AS INTEGER)) as avg_age FROM complex_json_test",
        [](const Result &result) {
...
void testDataTypesAndFunctions(DbClientPtr client)
{
    ...
    // 插入各种数据类型
    client->execSqlAsync(
        "INSERT INTO data_types_advanced VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        "?, ?)",
        [](const Result &result) {
    ...
    // 测试类型转换函数
    client->execSqlAsync(
        "SELECT CAST(text_val AS INTEGER) as cast_int, CAST(double_val AS "
        "VARCHAR) as cast_str FROM data_types_advanced WHERE id = ?",
        [](const Result &result) {
            bool success = result.size() == 1;
            if (success)
            {
    ...
    // 测试数学函数
    client->execSqlAsync(
        "SELECT ABS(double_val) as abs_val, ROUND(double_val, 2) as round_val, "
        "CEIL(double_val) as ceil_val FROM data_types_advanced WHERE id = ?",
        [](const Result &result) {
            bool success = result.size() == 1;
            if (success)
            {
        ...
        [](const std::exception_ptr &e) { printTestResult("数学函数", false); },
        1);

    // 测试字符串函数
    client->execSqlAsync(
        "SELECT LENGTH(text_val) as len, UPPER(text_val) as upper_val, "
        "LOWER(text_val) as lower_val FROM data_types_advanced WHERE id = ?",
        [](const Result &result) {
            bool success = result.size() == 1;
            if (success)
            {
...
void testPerformanceOptimizations(DbClientPtr client)
{
    ...
    // 创建大表进行性能测试
    client->execSqlAsync(
        "CREATE TABLE IF NOT EXISTS performance_large (id INTEGER PRIMARY KEY, "
        "name VARCHAR, value DOUBLE, category VARCHAR, created_date DATE)",
        [](const Result &result) { printTestResult("创建大表", true); },
        [](const std::exception_ptr &e) {
    ...
    for (int i = 0; i < 5000; ++i)
    {
        ...
        client->execSqlAsync(
            "INSERT INTO performance_large VALUES (?, ?, ?, ?, ?)",
            [](const Result &result) {
                // 成功回调
            },
            [](const std::exception_ptr &e) {
    ...
    printTestResult("批量插入性能",
                    insertDuration.count() < 2000);  // 应该在2秒内完成
    ...
    client->execSqlAsync(
        "SELECT category, COUNT(*) as count, AVG(value) as avg_value, "
        "MIN(value) as min_value, MAX(value) as max_value FROM "
        "performance_large GROUP BY category ORDER BY category",
        [start](const Result &result) {
            ...
            bool success = result.size() == 5 &&
                           queryDuration.count() < 500;  // 应该在500ms内完成
    ...
    // 测试索引效果
    client->execSqlAsync(
        "CREATE INDEX IF NOT EXISTS idx_category ON "
        "performance_large(category)",
        [](const Result &result) { printTestResult("创建索引", true); },
        [](const std::exception_ptr &e) {
    ...
    client->execSqlAsync(
        "SELECT * FROM performance_large WHERE category = ? LIMIT 100",
        [start](const Result &result) {
            auto end = std::chrono::high_resolution_clock::now();
            auto queryDuration =
                std::chrono::duration_cast<std::chrono::milliseconds>(end -
            ...
            bool success = result.size() == 100 &&
                           queryDuration.count() < 100;  // 应该在100ms内完成
...
int main(int argc, char *argv[])
{
    ...
    // 创建DuckDB客户端
    auto client = drogon::orm::DbClient::newDuckDbClient(
        "filename=duckdb_specific_test.db", 1);
