...
Task<> comprehensiveCoroTest(DbClientPtr client)
{
    try
    {
        ...
        // 2. 插入数据
        result = co_await client->execSqlCoro("INSERT INTO coro_test VALUES (1, 'test1', 10.5), (2, 'test2', 20.7)");
        printTestResult("协程-插入数据", result.affectedRows() == 2);
        
        // 3. 查询数据
        result = co_await client->execSqlCoro("SELECT * FROM coro_test WHERE id = ?", 1);
        printTestResult("协程-参数化查询", result.size() == 1 && result[0]["name"].as<std::string>() == "test1");
        
        // 4. JSON测试
        result = co_await client->execSqlCoro("CREATE TABLE IF NOT EXISTS coro_json_test (id INTEGER, data JSON)");
        result = co_await client->execSqlCoro("INSERT INTO coro_json_test VALUES (?, ?)", 1, R"({"name": "Alice", "age": 30})");
        result = co_await client->execSqlCoro("SELECT * FROM coro_json_test WHERE id = ?", 1);
        printTestResult("协程-JSON操作", result.size() == 1);
        
        // 5. 复杂查询
        result = co_await client->execSqlCoro("SELECT COUNT(*) as count, AVG(value) as avg_value FROM coro_test");
        printTestResult("协程-聚合查询", result.size() == 1);
        
        // 6. 事务测试
        result = co_await client->execSqlCoro("BEGIN TRANSACTION");
        result = co_await client->execSqlCoro("INSERT INTO coro_test VALUES (3, 'test3', 30.0)");
...
void testBasicOperations(DbClientPtr client) {
    ...
    client->execSqlAsync("CREATE TABLE IF NOT EXISTS basic_test (id INTEGER PRIMARY KEY, name VARCHAR, age INTEGER, salary DOUBLE)",
        [](const Result &result) {
            printTestResult("创建表", true);
        },
        [](const std::exception_ptr &e) {
            printTestResult("创建表", false);
        });
    
    // 2. 插入数据
    client->execSqlAsync("INSERT INTO basic_test VALUES (?, ?, ?, ?)",
        [](const Result &result) {
            printTestResult("插入数据", result.affectedRows() == 1);
        },
        [](const std::exception_ptr &e) {
            printTestResult("插入数据", false);
        }, 1, "Alice", 25, 50000.0);
    
    // 3. 查询数据
    client->execSqlAsync("SELECT * FROM basic_test WHERE id = ?",
        [](const Result &result) {
        ...
        [](const std::exception_ptr &e) {
            printTestResult("查询数据", false);
        }, 1);
    
    // 4. 更新数据
    client->execSqlAsync("UPDATE basic_test SET salary = ? WHERE id = ?",
        [](const Result &result) {
            printTestResult("更新数据", result.affectedRows() == 1);
        },
        [](const std::exception_ptr &e) {
            printTestResult("更新数据", false);
        }, 55000.0, 1);
    
    // 5. 删除数据
    client->execSqlAsync("DELETE FROM basic_test WHERE id = ?",
        [](const Result &result) {
            printTestResult("删除数据", result.affectedRows() == 1);
        },
        [](const std::exception_ptr &e) {
            printTestResult("删除数据", false);
        }, 1);
}
...
void testJsonOperations(DbClientPtr client) {
    ...
    // 2. 插入JSON数据
    std::string jsonData = R"({"name": "John", "age": 30, "city": "New York", "hobbies": ["reading", "swimming"]})";
    std::string metadata = R"({"created": "2024-01-01", "version": 1})";
    
    client->execSqlAsync("INSERT INTO json_test VALUES (?, ?, ?)",
        [](const Result &result) {
    ...
    // 3. 查询JSON数据
    client->execSqlAsync("SELECT * FROM json_test WHERE id = ?",
        [](const Result &result) {
            bool success = result.size() == 1 && 
                          !result[0]["data"].isNull() && 
    ...
    // 4. JSON路径查询
    client->execSqlAsync("SELECT data->>'$.name' as name, data->>'$.age' as age FROM json_test WHERE id = ?",
        [](const Result &result) {
            bool success = result.size() == 1 && 
    ...
    // 5. 使用SqlBinder插入JSON
    {
        ...
        binder << 2 << R"({"name": "Jane", "age": 25, "city": "London"})" << R"({"created": "2024-01-02", "version": 1})";
...
void testDataTypes(DbClientPtr client) {
    ...
    client->execSqlAsync("INSERT INTO data_types_test VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [](const Result &result) {
    ...
    // 3. 查询并验证数据类型
    client->execSqlAsync("SELECT * FROM data_types_test WHERE id = ?",
        [](const Result &result) {
            if (result.size() == 1) {
                auto row = result[0];
                bool success = row["int_val"].as<int>() == 42 &&
                              row["bigint_val"].as<int64_t>() == 123456789012345 &&
                              std::abs(row["double_val"].as<double>() - 3.14159) < 0.0001 &&
...
void testErrorHandling(DbClientPtr client) {
    ...
    // 2. 约束违反
    client->execSqlAsync("CREATE TABLE IF NOT EXISTS constraint_test (id INTEGER PRIMARY KEY, name VARCHAR UNIQUE)",
        [](const Result &result) {
            printTestResult("创建约束表", true);
        },
        [](const std::exception_ptr &e) {
            printTestResult("创建约束表", false);
        });
    
    client->execSqlAsync("INSERT INTO constraint_test VALUES (?, ?)",
        [](const Result &result) {
...
void testConcurrency(DbClientPtr client) {
    ...
    for (int i = 0; i < 10; ++i) {
        threads.emplace_back([client, i, &successCount, &totalCount]() {
            client->execSqlAsync("INSERT INTO concurrency_test VALUES (?, ?)",
    ...
    for (int i = 0; i < 10; ++i) {
        threads.emplace_back([client, i, &successCount, &totalCount]() {
            client->execSqlAsync("SELECT * FROM concurrency_test WHERE id = ?",
                [&successCount](const Result &result) {
                    if (result.size() == 1) {
                        successCount++;
                    }
...
void testPerformance(DbClientPtr client) {
    ...
    for (int i = 0; i < 1000; ++i) {
        client->execSqlAsync("INSERT INTO performance_test VALUES (?, ?, ?)",
            [](const Result &result) {
                // 成功回调
            },
            [](const std::exception_ptr &e) {
                // 失败回调
            }, i, "test" + std::to_string(i), i * 1.5);
    }
    ...
    printTestResult("批量插入性能测试", duration.count() < 1000); // 应该在1秒内完成
    
    // 查询性能测试
    start = std::chrono::high_resolution_clock::now();
    
    client->execSqlAsync("SELECT COUNT(*) as count, AVG(value) as avg_value FROM performance_test",
...
void testAdvancedFeatures(DbClientPtr client) {
    ...
    client->execSqlAsync("CREATE TABLE IF NOT EXISTS view_source (id INTEGER, name VARCHAR, category VARCHAR)",
        [](const Result &result) {
            printTestResult("创建视图源表", true);
        },
        [](const std::exception_ptr &e) {
            printTestResult("创建视图源表", false);
        });
    
    client->execSqlAsync("INSERT INTO view_source VALUES (1, 'Item1', 'A'), (2, 'Item2', 'B'), (3, 'Item3', 'A')",
        [](const Result &result) {
    ...
    client->execSqlAsync("CREATE VIEW category_view AS SELECT category, COUNT(*) as count FROM view_source GROUP BY category",
        [](const Result &result) {
            printTestResult("创建视图", true);
        },
        [](const std::exception_ptr &e) {
            printTestResult("创建视图", false);
        });
    
    client->execSqlAsync("SELECT * FROM category_view",
        [](const Result &result) {
    ...
    // 2. 复杂JSON查询
    client->execSqlAsync("CREATE TABLE IF NOT EXISTS complex_json (id INTEGER, data JSON)",
        [](const Result &result) {
    ...
    client->execSqlAsync("INSERT INTO complex_json VALUES (?, ?)",
        [](const Result &result) {
    ...
    client->execSqlAsync("SELECT data->>'$.user.name' as name, data->>'$.user.age' as age FROM complex_json WHERE id = ?",
        [](const Result &result) {
            bool success = result.size() == 1 && 
...
int main(int argc, char *argv[])
{
    ...
    // 创建DuckDB客户端
    auto client = drogon::orm::DbClient::newDuckDbClient("filename=comprehensive_test.db", 1);
