...
Result makeResult(std::shared_ptr<MYSQL_RES> &&r = nullptr,
  ...
  return Result{
      std::make_shared<MysqlResultImpl>(std::move(r), affectedRows, insertId)};
}
...
void MysqlConnection::handleCmd(int status) {
  switch (execStatus_) {
    ...
    case ExecStatus::StoreResult: {
      MYSQL_RES *ret;
      waitStatus_ = mysql_store_result_cont(&ret, mysqlPtr_.get(), status);
...
void MysqlConnection::continueSetCharacterSet(int status) {
  ...
  if (waitStatus_ == 0) {
    if (err) {
      LOG_ERROR << "Error(" << err << ") \"" << mysql_error(mysqlPtr_.get())
                << "\"";
      ...
      if (seekPos == std::string::npos) {
        ...
        switch (format[i]) {
          ...
          case internal::MySqlLong:
            sql_.append(std::to_string(*((int32_t *)parameters[i])));
            break;
          case internal::MySqlLongLong:
            sql_.append(std::to_string(*((int64_t *)parameters[i])));
            break;
...
void MysqlConnection::startStoreResult(bool queueInLoop) {
  ...
  if (waitStatus_ == 0) {
    ...
    if (queueInLoop) {
      loop_->queueInLoop(
          [thisPtr = shared_from_this(), ret] { thisPtr->getResult(ret); });
    } else {
      getResult(ret);
    }
  }
