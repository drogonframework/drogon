...
void Sqlite3Connection::init() {
  ...
  loop_->runInLoop([this, filename = std::move(filename)]() {
    ...
    connectionPtr_ =
        std::shared_ptr<sqlite3>(tmp, [](sqlite3 *ptr) { sqlite3_close(ptr); });
...
void Sqlite3Connection::execSqlInQueue(
  ...
  if (!stmtPtr) {
    ...
    if (!std::all_of(remaining, sql.data() + sql.size(), [](char ch) {
      ...
      auto exceptPtr = std::make_exception_ptr(
          SqlError("Multiple semicolon separated statements are unsupported",
                   std::string{sql}));
      exceptCallback(exceptPtr);
      idleCb_();
      return;
  ...
  for (int i = 0; i < (int)parameters.size(); ++i) {
    ...
    switch (format[i]) {
      ...
      case Sqlite3TypeText:
        bindRet =
            sqlite3_bind_text(stmt, i + 1, parameters[i], -1, SQLITE_STATIC);
        break;
  ...
  if (sqlite3_stmt_readonly(stmt)) {
    ...
    if (r == SQLITE_DONE) {
      ...
      resultPtr->insertId_ = sqlite3_last_insert_rowid(connectionPtr_.get());
    }
    sqlite3_reset(stmt);
  }
