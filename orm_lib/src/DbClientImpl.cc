  ...
  auto trans = std::make_shared<TransactionImpl>(
      type_, conn, std::function<void(bool)>(), [weakThis, conn]() {
        ...
        }
        {
          ...
          if (thisPtr->connections_.find(conn) == thisPtr->connections_.end()) {
...
DbConnectionPtr DbClientImpl::newConnection(trantor::EventLoop *loop) {
...
#if USE_POSTGRESQL
#if LIBPQ_SUPPORTS_BATCH_MODE
    connPtr = std::make_shared<PgConnection>(loop, connectionInfo_, autoBatch_);
#else
...
#if USE_SQLITE3
    auto sqlite3ConnPtr = std::make_shared<Sqlite3Connection>(loop,
                                                              connectionInfo_,
                                                              sharedMutexPtr_);
    sqlite3ConnPtr->init();
  ...
  connPtr->setOkCallback([weakPtr](const DbConnectionPtr &okConnPtr) {
    ...
    {
      std::lock_guard<std::mutex> guard(thisPtr->connectionsMutex_);
      thisPtr->busyConnections_.insert(okConnPtr);  // For new connections, this
