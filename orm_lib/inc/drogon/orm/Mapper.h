...
class Mapper {
 public:
  ...
  inline typename std::
      enable_if<!std::is_same<typename U::PrimaryKeyType, void>::value, T>::type
    ...
    static_assert(internal::has_sqlForFindingByPrimaryKey<T>::value,
  ...
  findFutureByPrimaryKey(const TraitsPKType &key) noexcept {
    ...
    binder >> [prom](const Result &r) {
      if (r.size() == 0) {
        ...
        prom->set_exception(
            std::make_exception_ptr(UnexpectedRows("Found more than one row")));
      } else {
...
inline Mapper<T> &Mapper<T>::orderBy(const std::string &colName,
  ...
  if (orderByString_.empty()) {
    orderByString_ = utils::formattedString(" order by %s", colName.c_str());
...
inline void Mapper<T>::deleteByPrimaryKey(
  ...
  auto binder = *client_ << T::sqlForDeletingByPrimaryKey();
  outputPrimeryKeyToBinder(key, binder);
  binder >> [rcb = std::move(rcb)](const Result &r) { rcb(r.affectedRows()); };
  binder >> ecb;
}
