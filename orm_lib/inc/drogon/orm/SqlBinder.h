 ...
 private:
  ...
  typename std::enable_if<(sizeof...(Values) < Boundary), void>::type run(
    ...
    if (row && row->size() > sizeof...(Values)) {
      ...
      value = makeValue<ValueType>((*row)[(Row::SizeType)sizeof...(Values)]);
    }
...
class DROGON_EXPORT SqlBinder : public trantor::NonCopyable {
 ...
 public:
  ...
  template <
      ...
      typename traits = FunctionTraits<typename std::decay<CallbackType>::type>>
  typename std::enable_if<traits::isExceptCallback && traits::isPtr, self>::type
      &
  ...
  self &operator<<(const Json::Value &j) noexcept(true) {
    switch (j.type()) {
      ...
      default:
        ...
        std::call_once(once_json, []() { jsonBuilder["indentation"] = ""; });
