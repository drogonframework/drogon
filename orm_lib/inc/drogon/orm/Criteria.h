...
class DROGON_EXPORT Criteria {
 public:
  ...
  Criteria(const std::string &colName, const CompareOperator &opera, T &&arg) {
    ...
    outputArgumentsFunc_ = [arg = std::forward<T>(arg)](
                               internal::SqlBinder &binder) { binder << arg; };
  }
  ...
  Criteria(const std::string &colName,
    ...
    assert(
        ((opera == CompareOperator::In) || (opera == CompareOperator::NotIn)) &&
        (argsSize > 0));
  ...
  Criteria(const std::string &colName,
    ...
    conditionString_.append(")");
    outputArgumentsFunc_ = [args =
                                std::move(args)](internal::SqlBinder &binder) {
      for (auto &arg : args) {
        binder << arg;
      }
    };
