...
#define SQLITE_SOURCE_ID sqlite3_sourceid()
    ...
    ** but are associated with the library instead of the header file.
    *^(Cautious
...
#define double sqlite3_int64
    ...
    ** [prepared statements], [sqlite3_blob_close | close] all [BLOB handles],
    *and
    ...
    ** it returns [SQLITE_OK] regardless, but instead of deallocating the
    *database
    ** connection immediately, it marks the database connection as an unusable
    ** "zombie" and makes arrangements to automatically deallocate the database
    ** connection after all prepared statements are finalized, all BLOB handles
    ** are closed, and all backups have finished. The sqlite3_close_v2()
    *interface
...
#define SQLITE_SYNC_DATAONLY 0x00010
    ...
    ** may be invoked even if the [sqlite3_vfs.xOpen] reported that it failed.
    *The
...
#define SQLITE_LAST_ERRNO SQLITE_FCNTL_LAST_ERRNO
    ...
    ** this even if the open fails.  SQLite expects that the
    ...
    */
    ...
    struct sqlite3_vfs
    {
        ...
        */
        ...
        /*
        ** The methods above are in versions 1 through 3 of the sqlite_vfs
        *object.
...
#define SQLITE_SHM_NLOCK 8
    ...
    ** that indicates what aspect of the [database connection] is being
    *configured.
...
#define SQLITE_DBCONFIG_MAX 1017                   /* Largest DBCONFIG */
    ...
    ** recorded. ^If no successful [INSERT]s into rowid tables have ever
    *occurred
    ...
    ** ^If it is used by the second or subsequent such statement within a
    *trigger
    ...
    ** SQL statements on [database connection] D complete.  ^Any new SQL
    *statements
    ...
    ** well-formed CREATE TRIGGER statement.  ^Semicolons that are embedded
    *within
    ...
    ** the built-in random() and randomblob() SQL functions.  This interface
    *allows
    ...
    ** the particular action to be authorized. ^The third through sixth
    *parameters
    ...
    ** [truncate optimization] is disabled and all rows are deleted
    *individually.
    **
    ** An authorizer is used when [sqlite3_prepare | preparing]
    ** SQL statements from an untrusted source, to ensure that the SQL
    *statements
    ** do not try to access data they are not allowed to see, or that they do
    *not
...
#define SQLITE_TRACE_CLOSE 0x08
    ...
    ** case the database must already exist, otherwise an error is
    *returned.</dd>)^
    **
    ...
    ** make use of additional special filenames that begin with the ":"
    *character.
    ...
    **
    ** <ul>
    **   <li> <b>vfs</b>: ^The "vfs" parameter may be used to specify the name
    *of
    **     a VFS object that provides the operating system interface that should
    **     be used to access the database file on disk. ^If this option is set
    *to
    ...
    **     access, just as if the [SQLITE_OPEN_READONLY] flag had been set in
    *the
    ...
    **
    **   <li> <b>cache</b>: ^The cache parameter may be set to either "shared"
    *or
    ...
    **          Open file "/home/fred/data.db". Use the special VFS
    *"unix-dotfile"
    ...
    **
    ** <b>Note to Windows users:</b>  The encoding used for the filename
    *argument
    ...
    ** sqlite3_create_filename(D,J,W,N,P) is a pointer to a database filename
    *that
    ...
    ** return a NULL pointer.  The memory obtained from
    *sqlite3_create_filename(X)
    ** must be released by a corresponding call to sqlite3_free_filename(Y).
    **
    ** The P parameter in sqlite3_create_filename(D,J,W,N,P) should be an array
    ** of 2*N pointers to strings.  Each pair of pointers in this array
    *corresponds
    ...
    ** that if the [sqlite3_vfs.xOpen()] method of a VFS has been called using
    *Y,
...
#define SQLITE_PREPARE_NO_VTAB 0x04
    ...
    ** METHOD: sqlite3_stmt
    **
    ** ^(In the SQL statement text input to [sqlite3_prepare_v2()] and its
    *variants,
    ...
    ** NULL.  ^These routines might also return NULL if a memory allocation
    *error
    ** occurs.  ^Otherwise, they return the name of the attached database,
    *table,
...
#define SQLITE3_TEXT 3
    ...
    ** should be returned. ^The leftmost column of the result set has the index
    *0.
    ...
    ** is used to extract the value) then an automatic type conversion is
    *performed.
    **
    ...
    ** value from sqlite3_column_blob() for a zero-length BLOB is a NULL
    *pointer.
    **
    ...
    *xStep
    ** and xFinal and NULL pointer must be passed for xFunc. ^To delete an
    *existing
    ...
    ** non-NULL, in which case the new function may be used as either an
    *aggregate
...
*/
    ...
    ** <td>&rarr;&nbsp;&nbsp;<td>True if value originated from a [bound
    *parameter]
    ** </table></blockquote>
    **
    ** <b>Details:</b>
    ...
    ** is not threadsafe.
    **
    ** ^These routines work just like the corresponding [column access
    *functions]
    ** except that these routines take a single [protected sqlite3_value] object
    ** pointer instead of a [sqlite3_stmt*] pointer and an integer column
    *number.
    **
    ...
    ** routine is part of the [pointer passing interface] added for
    *SQLite 3.20.0.
    **
    ...
    ** occurs is undefined and may change from one release of SQLite to the
    *next.
    **
    ...
    ** to be a NULL value.  If sqlite3_value_nochange(X) is invoked anywhere
    *other
    ...
    ** object D and returns a pointer to that copy.  ^The [sqlite3_value]
    *returned
    ...
    **
    ** ^The sqlite3_set_auxdata(C,N,P,X) interface saves P as metadata for the
    *N-th
    ...
    ** final argument to routines like [sqlite3_result_blob()].  ^If the
    *destructor
...
#define SQLITE_TRANSIENT ((sqlite3_destructor_type)-1)
    ...
    ** result of expressions operating on strings with embedded NULs is
    *undefined.
    ...
    ** and a UTF-16 string in native byte order for
    ...
    ** function callback are the length of the two strings, in bytes. The
    *collating
    ...
    ** compatibility.
    **
    ** See also:  [sqlite3_collation_needed()] and
    ...
    ** ^A call to either function replaces the existing collation-needed
    *callback.
    **
    ...
    ** all, then the behavior of sqlite3_sleep() may deviate from the
    *description
    ...
    ** [sqlite3_win32_set_directory] interface is used to set the value
    *associated
    ** with the [sqlite3_temp_directory] or [sqlite3_data_directory] variable,
    *to
    ** zValue, depending on the value of the type parameter.  The zValue
    *parameter
    ** should be NULL to cause the previous value to be freed via
    *[sqlite3_free];
    ** a non-NULL value will be copied into memory obtained from
    *[sqlite3_malloc]
    ** prior to being used.  The [sqlite3_win32_set_directory] interface returns
    ** [SQLITE_OK] to indicate success, [SQLITE_ERROR] if the type is
    *unsupported,
...
#define SQLITE_WIN32_TEMP_DIRECTORY_TYPE 2
    ...
    ** function to be invoked whenever a transaction is [ROLLBACK | rolled
    *back].
    ...
    **
    ** ^The sqlite3_commit_hook(D,C,P) and sqlite3_rollback_hook(D,C,P)
    *functions
    ...
    **
    ** ^(This routine returns [SQLITE_OK] if shared cache was enabled or
    *disabled
    ...
    **      [sqlite3_config]([SQLITE_CONFIG_MEMSTATUS],...) start-time option
    *and
    ...
    ** <tr><td> 7th <td> int         <td> True if column has a NOT NULL
    *constraint
    ** <tr><td> 8th <td> int         <td> True if column is part of the PRIMARY
    *KEY
    ...
    ** [INTEGER PRIMARY KEY] column has been explicitly declared, then the
    *output
    ...
    ** [SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION] method be used to enable only
    *this
    ...
    ** xEntryPoint() is the entry point for a statically linked [SQLite
    *extension]
    ...
    **
    ** where OP is =, &lt;, &lt;=, &gt;, or &gt;=.)^  ^(The particular operator
    *is
    ...
    ** checked separately in byte code.  If the omit flag is change to true,
    *then
    ...
    **
    ** ^The orderByConsumed means that output from [xFilter]/[xNext] will occur
    *in
    ...
    *the
    ** implementation returns SQLITE_CONSTRAINT, then there is no need to
    *rollback
...
#define SQLITE_INDEX_CONSTRAINT_FUNCTION 150
    ...
    ** is delivered up to the client application, the string will be
    *automatically
    ...
    ** before this API is called, a new function is created.)^  ^The
    *implementation
    ...
    ** ^The [sqlite3_blob_bytes()] interface returns the size of the BLOB in
    *bytes.
    */
    ...
    ** code is SQLITE_MISUSE, *ppBlob is set to NULL.)^ ^This means that,
    *provided
    ...
    **         column zColumn is part of a [child key] definition and the blob
    *is
    ...
    ** METHOD: sqlite3_blob
    **
    ** ^This function is used to move an existing [BLOB handle] so that it
    *points
    ...
    ** the nominated column.)^ ^If the new row is not present in the table, or
    *if
    ...
    ** the database is in auto-commit mode and there are no other open
    *read-write
...
#define SQLITE_TESTCTRL_LAST 29 /* Largest TESTCTRL */
    ...
    ** <li> Put identifier names inside &#91;...&#93;.  This is not standard
    *SQL,
    ...
    ** a new [sqlite3_str] object.  To avoid memory leaks, the object returned
    *by
    ...
    ** and returns a pointer to a memory buffer obtained from
    *[sqlite3_malloc64()]
    ...
    **
    ** These methods do not return a result code.  ^If an error occurs, that
    *fact
...
#define SQLITE_STMTSTATUS_MEMUSED 99
    ...
    ** it is purely advisory.  ^On a cache where bPurgeable is false, SQLite
    *will
    ...
    ** discarded. ^Any prior cache entry associated with newKey is guaranteed
    *not
    ** to be pinned.
    **
    ...
    ** free up as much of heap memory as possible.  The page cache
    *implementation
    ** is not obligated to free any memory, but well-behaved implementations
    *should
    ** do their best.
    */
    ...
    **         backup,
    **     <li><b>sqlite3_backup_step()</b> is called one or more times to
    *transfer
    ...
    ** the source and destination databases specified by [sqlite3_backup] object
    *B.
    ...
    ** [SQLITE_DONE], a call to sqlite3_backup_step() may return
    *[SQLITE_READONLY],
    ...
    **
    ** ^The source [database connection] may be used by the application for
    *other
    ...
    ** if the application incorrectly accesses the destination [database
    *connection]
    ...
    ** not just the specific connection that was passed to
    *sqlite3_backup_init().
    **
    ...
    ** operator in the SQL dialect understood by SQLite.  ^For "X LIKE P"
    *without
    ...
    ** either "main" or the name of an [ATTACH]-ed database. ^The fourth
    *parameter
    ...
    ** occur.  But then the newer and more powerful
    ...
    **   database writer attempts while it is pending, but does not impede
    *readers.
    **
    ...
    ** of an error or because the database is not in [WAL mode]. ^If pnCkpt is
    *not
    ** NULL,then *pnCkpt is set to the total number of checkpointed frames in
    *the
    ** log file (including any that were already checkpointed before the
    *function
    ** was called) or to -1 if the checkpoint could not run due to an error or
    ** because the database is not in WAL mode. ^Note that upon successful
    ** completion of an SQLITE_CHECKPOINT_TRUNCATE, the log file will have been
    ** truncated to zero bytes and so both *pnLog and *pnCkpt will be set to
    *zero.
    **
    ...
    ** exclusive "writer" lock on the database file. ^If the writer lock cannot
    *be
    ** obtained immediately, and a busy-handler is configured, it is invoked and
    ** the writer lock retried until either the busy-handler returns 0 or the
    *lock
    ** is successfully obtained. ^The busy-handler is also invoked while waiting
    *for
    ** database readers as described above. ^If the busy-handler returns 0
    *before
    ...
    ** error occurs while processing an attached database, processing is
    *abandoned
...
#define SQLITE_CHECKPOINT_TRUNCATE 3 /* Like RESTART but also truncate WAL */
    ...
    ** of parameters after C depend on which [virtual table configuration
    *option]
    ** is used.
    */
...
#define SQLITE_VTAB_DIRECTONLY 3
    ...
    ** of the SQL statement that triggered the call to the [xUpdate] method of
    *the
    ** [virtual table].
    */
    ...
    ** a return value that is less expensive to compute and that the
    *corresponding
    ...
    ** structure passed to xBestIndex. This function returns a pointer to a
    *buffer
...
#define SQLITE_SCANSTAT_SELECTID 5
    ...
    ** Loops are numbered starting from zero. ^If idx is out of range - less
    *than
    ** zero or greater than or equal to the total number of loops used to
    *implement
    ...
    *cases
    ** where there exist loops with no available statistics, this function
    *behaves
    ...
    ** abandoned and an SQLite [error code] is returned to the caller
    *immediately.
    **
    ** ^Otherwise, if no error occurs, [sqlite3_db_cacheflush()] returns
    *SQLITE_OK.
    **
...
*/
    ...
    **        file immediately after it is first opened. At least one
    *transaction
    ...
    */
    SQLITE_API unsigned char *sqlite3_serialize(
        ...
        const char
            *zSchema, /* Which DB to serialize. ex: "main", "temp", ... */
...
extern "C"
{
    ...
    ** module function, including [sqlite3session_delete()] on the session
    *object
    ** are undefined.
    **
    ...
    */
    SQLITE_API int sqlite3session_enable(sqlite3_session *pSession,
                                         int bEnable);
    ...
    ** to the session object passed as the first argument. All subsequent
    *changes
    ...
    *explicitly
    ** defined as part of their CREATE TABLE statement. It does not matter if
    *the
    ...
    *rows
    ** in tables that are not attached to the Session object, the filter is
    *called
    ** to determine whether changes to the table's rows should be tracked or
    *not.
    ...
    *changes,
    ** each representing a change to a single row of an attached table. An
    *INSERT
    ** change contains the values of each field of a new database row. A DELETE
    ** contains the original values of each field of a deleted database row. An
    ** UPDATE change contains the original values of each field of an updated
    ** database row along with the updated values for each updated
    *non-primary-key
    ** column. It is not possible for an UPDATE change to represent a change
    *that
    ...
    ** PRIMARY KEY columns is updated so that all PRIMARY KEY columns are
    *non-NULL,
    ...
    ** a changeset or when applying a changeset to a database, all changes
    *related
    ** to a single table are processed before moving on to the next table.
    *Tables
    ** are sorted in the same order in which they were attached (or
    *auto-attached)
    ...
    **        change is added to the changeset. If no such row is found, no
    *change
    ...
    **        change is added to the changeset. If there is a row with a
    *matching
    ...
    *later
    ** deleted while a session object is active, neither the insert nor the
    *delete
    ...
    ** changeset, even though the delete took place while the session was
    *disabled.
    ...
    ** does not have a primary key, this function is a no-op (but does not
    *return
    ** an error).
    **
    ...
    *tables
    ** are compatible but do not have any PRIMARY KEY columns, it is not an
    *error
    ...
    **
    **   <li> For each row (primary key) that exists in both tables, but
    *features
    ...
    **
    ** If the operation is successful, SQLITE_OK is returned. Otherwise, an
    *SQLite
    ...
    **
    ** Note that the sqlite3changeset_start_v2() API is still
    *<b>experimental</b>
...
#define SQLITE_CHANGESETSTART_INVERT 0x0002
    ...
    ** passed to a conflict-handler by [sqlite3changeset_apply()], or an
    *iterator
    ** created by [sqlite3changeset_start()]. In the latter case, the most
    *recent
    ** call to [sqlite3changeset_next()] must have returned [SQLITE_ROW]. If
    *this
    ...
    ** [sqlite3changeset_apply()], [SQLITE_MISUSE] is immediately returned and
    *the
    ** call has no effect.
    **
    ** If an error was encountered within a call to an sqlite3changeset_xxx()
    ** function (for example an [SQLITE_CORRUPT] in [sqlite3changeset_next()] or
    *an
    ** [SQLITE_NOMEM] in [sqlite3changeset_new()]) then an error code
    *corresponding
    ...
    ** a pointer to a new sqlite3_changegroup object before returning. The
    *caller
    ...
    ** callback". If it is not NULL, then for each table affected by at least
    *one
    ...
    ** changes associated with the table are applied. A warning message is
    *issued
    ** via the sqlite3_log() mechanism with the error code SQLITE_SCHEMA. At
    *most
    ...
    ** invoked. A description of exactly when the conflict handler is invoked
    *for
    ** each type of change is below.
    **
    ** Unlike the xFilter argument, xConflict may not be passed NULL. The
    *results
    ...
    ** SQLITE_CHANGESET_DATA or SQLITE_CHANGESET_CONFLICT. If the
    *conflict-handler
    ...
    **   the conflict-handler function is invoked with
    *[SQLITE_CHANGESET_NOTFOUND]
    **   passed as the second argument.
    **
    **   If the DELETE operation is attempted, but SQLite returns
    *SQLITE_CONSTRAINT
    ...
    **   This includes the case where the INSERT operation is re-attempted
    *because
    ...
    ** resolution strategy.
    **
    ** All changes made by these functions are enclosed in a savepoint
    *transaction.
    ...
    ** the input is a changeset (not a patchset), then
    *sqlite3changeset_apply_v2()
    ** may set (*ppRebase) to point to a "rebase" that may be used with the
    ** sqlite3_rebaser APIs buffer before returning. In this case (*pnRebase)
    ** is set to the size of the buffer in bytes. It is the responsibility of
    *the
    ** caller to eventually free any such buffer using sqlite3_free(). The
    *buffer
    ...
    ** may be modified by passing a combination of
    ** [SQLITE_CHANGESETAPPLY_NOSAVEPOINT | supported flags] as the 9th
    *parameter.
    **
    ** Note that the sqlite3changeset_apply_v2() API is still
    *<b>experimental</b>
    ** and therefore subject to change.
    */
    SQLITE_API int sqlite3changeset_apply(
        ...
        int (*xConflict)(
            ...
            sqlite3_changeset_iter
                *p /* Handle describing change and conflict */
            ),
...
#define SQLITE_CHANGESET_ABORT 2
    ...
    **   <li> Each local changeset is rebased by calling
    ...
    ** before returning SQLITE_OK. If the input is completely exhausted,
    *(*pnData)
    ...
    ** iterator. If such an xInput callback returns an error, the iterator
    *enters
    ...
    **    and output data in approximately 1 KiB chunks. This operand may be
    *used
    **    to set and query the value of this configuration setting. The pointer
    **    passed as the second argument must point to a value of type (int).
    **    If this value is greater than 0, it is used as the new streaming data
    **    chunk size for both input and output. Before returning, the (int)
    *value
...
extern "C"
{
    ...
    **   if an error occurs, an SQLite error code is returned and the final
    *values
    ...
    *function's
    **   "auxiliary data". The pointer may then be retrieved by the current or
    *any
    ...
    **
    **   This function is used to retrieve the total number of rows in the
    *table.
    ...
    *xPhraseNext
    **   method, to iterate through all instances of a single query phrase
    *within
    **   the current row. This is the same information as is accessible via the
    **   xInstCount/xInst APIs. While the xInstCount/xInst APIs are more
    *convenient
    ...
    **   of iterating through all instances of a phrase in the current row,
    *these
    ...
    **   four values:
    **
    **   <ul><li> <b>FTS5_TOKENIZE_DOCUMENT</b> - A document is being inserted
    *into
    **            or removed from the FTS table. The tokenizer is being invoked
    *to
    **            determine the set of tokens to add to (or delete from) the
    **            FTS index.
    **
    **       <li> <b>FTS5_TOKENIZE_QUERY</b> - A MATCH query is being executed
    **            against the FTS index. The tokenizer is being called to
    *tokenize
    **            a bareword or quoted string specified as part of the query.
    **
    **       <li> <b>(FTS5_TOKENIZE_QUERY | FTS5_TOKENIZE_PREFIX)</b> - Same as
    **            FTS5_TOKENIZE_QUERY, except that the bareword or quoted string
    *is
    ...
    **            function. Or an fts5_api.xColumnSize() request made by the
    *same
    ...
    **   size of the token in bytes. The 4th and 5th arguments are the byte
    *offsets
    ...
    **   built-in tokenizers, the FTS5 query 'first + place' will match
    *instances
    ...
    **            fact "first", so that when the user inserts the document "I
    *won
    **            1st place" entries are added to the index for tokens "i",
    *"won",
    **            "first" and "place". If the user then queries for '1st +
    *place',
    **            the tokenizer substitutes "first" for "1st" and the query
    *works
    **            as expected.
    **
    ...
    **
    **            the tokenizer offers both "1st" and "first" as synonyms for
    *the
    **            first token in the MATCH query and FTS5 effectively runs a
    *query
    **            similar to:
    **
    **   <codeblock>
    ...
    **
    **            except that, for the purposes of auxiliary functions, the
    *query
    **            still appears to contain just two phrases - "(first OR 1st)"
    **            being treated as a single phrase.
    **
    **       <li> By adding multiple synonyms for a single term to the FTS
    *index.
    **            Using this method, when tokenizing document text, the
    *tokenizer
    ...
    **   when parsing the document "I won first place", a tokenizer that
    *supports
    ...
    **   xToken() is called. Multiple synonyms may be specified for a single
    *token
    ...
    **   extra data to the FTS index or require FTS5 to query for multiple
    *terms,
    **   so it is efficient in terms of disk space and query speed. However, it
    **   does not support prefix queries very well. If, as suggested above, the
    **   token "first" is substituted for "1st" by the tokenizer, then the
    *query:
    **
    **   <codeblock>
    **     ... MATCH '1s*'</codeblock>
